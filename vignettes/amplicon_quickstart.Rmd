...
if (Sys.getenv("GITHUB_ACTIONS") != ""){
  ...
  get_table_from_database("samples")
  frozen_set_ag <- samples
} else {
  connect_database() 
...
```{r}
phy_amplicon <- vdb_make_phylo(frozen_set_ag %>% dplyr::sample_n(100)) %>% 
...
mypal <- make_microviz_palette(phy_amplicon, n=30, rank="genus", color_db=asv_annotation_blast_color_ag) 
phy_amplicon %>%
  comp_barplot(tax_level = "genus", palette = mypal, n_taxa=30 ) +
...
```{r}
tsne <- make_tsne(dists) %>% 
  left_join(as_tibble(sample_data(phy_amplicon)) %>% rename(sampleid = .sample))
