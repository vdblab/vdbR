...
vignette: >
  %\VignetteIndexEntry{mgx-quickstart}
...
MB_H001440_MC02	GutZymo1	MOCK_CONTROL
...
zymo_qc <- mgx_qc %>% filter(ia_id %in% zymo_preprocess_analyses$ia_id)
...
>We should proabably make this an argument to vdb_make_phylo_mgx, allowing users to use either relative abundances or attributed counts data type.

```{r}
phymgx <- vdb_make_phylo_mgx(metadata, sampleid_col = "SampleID", app_id = 66) %>% 
  speedyseq::transform_sample_counts(fun = function(x) round(x*1e6, 0))
phymgx
...
yt_et_al_pal <-  make_microviz_palette(phymgx_genus, n=20, rank='genus', color_db=asv_annotation_blast_color_ag)
